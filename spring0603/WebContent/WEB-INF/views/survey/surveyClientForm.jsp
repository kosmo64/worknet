 ...
 <%-- Content 내용들어 갈자리 --%>   
	...
	<table id="survey">
		...
		<tbody>
		   ...
		   <c:if test="${i.index == 0 }">
		   <tr><th>제목</th><td><input type="text" name="sub" id="sub" value="${e.sub }">
		    <input type="hidden" name="subcode" value="${e.num }">
		   </td></tr>
 		   </c:if>
			<tr><th>${i.index+1}번 설문문항 ${e.surveytype }</th>
			<td><input type="radio" name="surveytype" class="surveytitle" 
			value="${e.surveytype }">
			${e.surveytitle }
			</td></tr>
			</c:forEach>
